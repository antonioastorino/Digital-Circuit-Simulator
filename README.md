...
build/dcs-<OPT_LEVEL> [<PRJ_NUM>]      # run the project number <PRJ_NUM>  
...
```
make SHELL=/bin/bash [OPT=<OPT_LEVEL>]  # build with optimization level <OPT_LEVEL>, 0 by default
build/dcs-<OPT_LEVEL> [<TEST_NUM>]      # run the test number <TEST_NUM>
...
- the header file must be included in `src/main.cpp`
- the function that represents the entry point to the project must be listed in `main()` of `src/main.cpp`
- the function that represents the entry point to the test must be listed in `main()` of `src/main-test.cpp`
